...
const Chat = ({ chat }) => {
    ...
    const onSend = async () => {
        await connection.invoke('SendMessage', { text: text, chatId: chat.chatId, roomId: chat.roomId, status: 1 });
    };
    useEffect(() => {
    ...
    return (
        <div class="flex-1 justify-between flex flex-col h-screen">
            ...
            {loading ? (
                ...
                >
                    {messages.map((m) => {
                        return <Message isMe={m.isMe} message={m.text} avatar={m.userAvatar} />;
                    })}
                    <div ref={divRef} />
                </div>
            )}
            <div class="border-t-2 border-gray-200 p-4 mb-2 sm:mb-0">
